...
Namespace IRC
    Class IRCCommandResolver
        ...
        Public CommandPrefixes As String() = {"%", "/", "pepinos:"}

        Sub New(ByVal prefixes As String())
            CommandPrefixes = prefixes
            Clist = CommandList()
        End Sub


        Function ResolveCommand(ByVal ttext As String, ByVal BOTIRCNickName As String, ByRef WorkerBot As Bot, ByVal source As String, client As IRC_Client, nickname As String) As IRCMessage
            client = client
            _bot = WorkerBot
            Dim tline As String = ":" & nickname & "!TELEGRAM@TELEGRAM PRIVMSG " & source & " :" & ttext
            Dim ARGS As New IRCCommandParams(tline, client, _bot, client.TaskAdm)

            If Not BeginsWithPrefix(CommandPrefixes, ARGS.MessageLine) Then Return Nothing
            Dim requestedCommand As String = RemovePrefix(ARGS.CommandName)
            For Each Command As IRCCommand In Clist
                If Command.Aliases.Contains(requestedCommand) Then
                    EventLogger.Log(String.Format(IRCMessages.CommandIssued, requestedCommand, ARGS.Realname, ARGS.CParam), ARGS.Source, ARGS.Realname)
                    Return Command.ComFunc(ARGS)
                End If
            Next
            If ARGS.MessageLine.ToLower.Contains(BOTIRCNickName.ToLower) Then
        ...
        Function ResolveCommand(ByVal imputline As String, ByVal BOTIRCNickName As String, ByRef IRCCLient As IRC_Client, ByRef WorkerBot As Bot) As IRCMessage
            Client = IRCCLient
            _bot = WorkerBot
            Dim arg As New IRCCommandParams(imputline, Client, _bot, Client.TaskAdm)
            If Not BeginsWithPrefix(CommandPrefixes, arg.MessageLine) Then
                Do While Client.Messages.Count > 100
                    Client.Messages.RemoveAt(0)
                Loop
                If Not String.IsNullOrWhiteSpace(arg.MessageLine) Then
                    Client.Messages.Add(New Tuple(Of String, String, String())(arg.Source, arg.Realname, {arg.MessageLine}))
                End If
                Return Nothing
            End If
            ...
            For Each Command As IRCCommand In Clist
                If Command.Aliases.Contains(requestedCommand) Then
                    EventLogger.Log(String.Format(IRCMessages.CommandIssued, requestedCommand, arg.Realname, arg.CParam), arg.Source, arg.Realname)
                    Return Command.ComFunc(arg)
                End If
            Next
            If arg.MessageLine.ToLower.Contains(BOTIRCNickName.ToLower) Then
        ...
        Private Function CommandInfoFcn(ByVal Params As IRCCommandParams) As IRCMessage
            For Each c As IRCCommand In Clist
                If c.Aliases.Contains(RemovePrefix(Params.CParam).ToLower) Then
                    Return New IRCMessage(Params.Source, "Comando: " & ColoredText(c.Name, 4) & "| Aliases: " & ColoredText(String.Join("/"c, c.Aliases), 3) & "| Descripción: " & c.Description & "| Uso: " & ColoredText(Params.CommandName & c.Usage, 10))
                End If
            Next
